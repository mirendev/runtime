...
func TestInitialPassMigratesCanonicalGraphAndProvenance(t *testing.T) {
	...
	dep := &core_v1alpha.Deployment{
		...
		Status: "active", Phase: "activating",
	...
	controller := New(log, inmem.Store, inmem.EAC, func() { sweepCompleteCount++ })
	...
	assert.Equal(t, finished, migratedDep.FinishedAt)
	...
	assert.Equal(t, 1, sweepCompleteCount)
	for range 4 {
		require.NoError(t, controller.Step(ctx))
	}
	assert.Equal(t, 1, sweepCompleteCount, "initial completion stays latched across later reconciliation sweeps")
}
...
func TestDeploymentMigrationRediscoversProgressAfterRestart(t *testing.T) {
	...
	first := New(log, inmem.Store, inmem.EAC, nil)
	...
	second := New(log, inmem.Store, inmem.EAC, nil)
...
func TestVersionMigrationIgnoresDeploymentsWithoutProvenance(t *testing.T) {
	...
	require.NoError(t, err)
	controller := New(log, inmem.Store, inmem.EAC, nil)
...
func TestMigrationFailureDoesNotBlockLaterPhasesOrReleaseInitialSweep(t *testing.T) {
	ctx := context.Background()
	inmem, cleanup := testutils.NewInMemEntityServer(t)
	t.Cleanup(cleanup)
	log := slog.New(slog.NewTextHandler(io.Discard, nil))
	...
	controller := New(log, failing, inmem.EAC, func() { sweepComplete = true })
	controller.Config.BatchSize = 100
	require.Error(t, controller.Step(ctx))
	for range 3 {
		require.NoError(t, controller.Step(ctx))
	}
	assert.False(t, sweepComplete,
		"touching every record is not enough when one failed to migrate")
