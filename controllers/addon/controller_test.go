...
func fixedService(name string) core_v1alpha.ConfigSpecServices {
...
func TestApplyAddonDisksAttachesToEveryService(t *testing.T) {
	spec := &core_v1alpha.ConfigSpec{
		Services: []core_v1alpha.ConfigSpecServices{fixedService("web"), fixedService("worker")},
	}

	require.NoError(t, applyAddonDisks(spec, []addon.Disk{sqliteDisk()}))

	for _, svc := range spec.Services {
		require.Len(t, svc.Disks, 1, "service %q", svc.Name)
		require.Equal(t, "sqlite", svc.Disks[0].Name)
		require.Equal(t, "addon", svc.Disks[0].Source)
		require.Equal(t, core_v1alpha.ConfigSpecServicesDisksSQLITE, svc.Disks[0].Provider)
	}
}

// Re-running provisioning must not stack duplicate disks.
func TestApplyAddonDisksIsIdempotent(t *testing.T) {
	spec := &core_v1alpha.ConfigSpec{Services: []core_v1alpha.ConfigSpecServices{fixedService("web")}}
...
// A disk the user declared by the same name is not silently displaced, and the
// addon must not report success while the app holds a path to storage that was
// never attached.
func TestApplyAddonDisksRejectsUserDeclaredName(t *testing.T) {
	svc := fixedService("web")
	...
	err := applyAddonDisks(spec, []addon.Disk{sqliteDisk()})
	require.Error(t, err)
	require.Contains(t, err.Error(), "already declares a disk by that name")

	// The user's disk is untouched.
	require.Len(t, spec.Services[0].Disks, 1)
	require.Equal(t, "/somewhere-else", spec.Services[0].Disks[0].MountPath)
}

// SQLite allows one writer, so attaching to a service that can scale would
// corrupt it. Refusing is better than the launcher silently skipping the disk.
func TestApplyAddonDisksRefusesScalableService(t *testing.T) {
	spec := &core_v1alpha.ConfigSpec{
		Services: []core_v1alpha.ConfigSpecServices{{
			Name: "web",
			Concurrency: core_v1alpha.ConfigSpecServicesConcurrency{
				Mode: "auto", RequestsPerInstance: 10,
			},
		}},
	}
	...
	err := applyAddonDisks(spec, []addon.Disk{sqliteDisk()})
	require.Error(t, err)
	require.Contains(t, err.Error(), "only one writer")
	require.Empty(t, spec.Services[0].Disks)
}

// An empty Services list means every service; naming one narrows it.
func TestApplyAddonDisksHonorsServiceSelection(t *testing.T) {
	spec := &core_v1alpha.ConfigSpec{
		Services: []core_v1alpha.ConfigSpecServices{fixedService("web"), fixedService("worker")},
	}

	d := sqliteDisk()
	d.Services = []string{"worker"}
	require.NoError(t, applyAddonDisks(spec, []addon.Disk{d}))

	require.Empty(t, spec.Services[0].Disks, "web was not selected")
	require.Len(t, spec.Services[1].Disks, 1, "worker was selected")
}
