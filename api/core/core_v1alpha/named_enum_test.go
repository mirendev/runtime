...
func TestNamedEnumEncodesLegacyGoValuesFromSuspendedSaga(t *testing.T) {
	legacy := ConfigSpec{
		Services: []ConfigSpecServices{{
			Disks: []ConfigSpecServicesDisks{{
				Provider: ConfigSpecServicesDisksProvider("component.config_spec.services.disks.provider.local"),
			}},
			Ports: []ConfigSpecServicesPorts{{
				Protocol: ConfigSpecServicesPortsProtocol("component.config_spec.services.ports.protocol.udp"),
			}},
		}},
	}
	...
	var resumed ConfigSpec
	require.NoError(t, json.Unmarshal(data, &resumed))

	diskAttr, ok := entity.New(resumed.Services[0].Disks[0].Encode()).Get(ConfigSpecServicesDisksProviderId)
	require.True(t, ok)
	assert.Equal(t, DiskProviderLocalMemberId, diskAttr.Value.Id())

	portAttr, ok := entity.New(resumed.Services[0].Ports[0].Encode()).Get(ConfigSpecServicesPortsProtocolId)
	require.True(t, ok)
	assert.Equal(t, PortProtocolUdpMemberId, portAttr.Value.Id())
}
