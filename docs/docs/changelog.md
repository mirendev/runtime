...
- **Requests to addon-backed services stop timing out** - An app with a `[addons.miren-postgresql]` addon could deploy and report healthy while every request touching the database timed out at the ingress. The service chain existed but held no rules, from two causes together: the chain body referenced a counter that hadn't been declared in the same transaction, and our cache recorded rules that nftables had rejected, so the next pass saw nothing to do. A rejected batch now clears the cache, and a chain declares the counter it uses. ([#1165](https://github.com/mirendev/runtime/pull/1165))
