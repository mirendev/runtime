...
- **Upgrade from Miren Cloud** - A cluster's page in Miren Cloud now shows when a newer release is out and upgrades the cluster with one click, running the same verified, roll-back-on-failure upgrade as `sudo miren upgrade` and following it through the restart. Automatic upgrades are there too, off until you turn them on: pick the Stable channel, which takes a release once it has proven out (usually about a week after it ships), or Latest, plus a maintenance window. Organization admins can send the outcome of every upgrade to Slack. Clusters need v0.16.0 to be upgraded from Cloud, so the upgrade to this release is the last one done by hand. See [Upgrades from Miren Cloud](./miren-cloud/upgrades.md). ([#1218](https://github.com/mirendev/runtime/pull/1218))
- **Upgrades you can walk away from, for the whole cluster** - `sudo miren upgrade` now runs as a durable operation on the host, so a dropped SSH session no longer leaves it in an unknown state. It succeeds only when a new server process reports ready on the target build, and otherwise rolls back to the previous binary and an etcd snapshot taken just before the restart. Then it upgrades your runners one at a time, stopping if the first one fails, and brings your CLI onto the same build. `miren cluster restart` and `miren cluster upgrade` do the same from any client, including for Docker installs, and `miren server operations list|show` shows what ran. For this one release, each runner needs its own `sudo miren upgrade` before the coordinator can manage it. ([#1216](https://github.com/mirendev/runtime/pull/1216), [#1217](https://github.com/mirendev/runtime/pull/1217), [#1218](https://github.com/mirendev/runtime/pull/1218), [#1238](https://github.com/mirendev/runtime/pull/1238), [#1242](https://github.com/mirendev/runtime/pull/1242))
- **Serve static sites straight from ingress** - Set `static.dir` in `app.toml` and ingress serves those files directly, with no running sandbox. A site whose files are already in the repo doesn't need an image either. See [Static sites & SPAs](./guides/static.md). ([#1251](https://github.com/mirendev/runtime/pull/1251))
- **Deploy targets checked into the repo** - `.miren/deploy.toml` names the clusters an app deploys to, so `miren deploy staging` picks one without `-C`. Targets resolve by Miren Cloud cluster ID, so the file works for teammates with different local cluster names. Manage it with `miren deploy target`. ([#1234](https://github.com/mirendev/runtime/pull/1234))

**Improvements**
- **Memory protection for the server process** - The server now runs under a memory limit of a quarter of the machine's RAM (at least 2 GB). If it ever grows past that, the kernel restarts it rather than letting it compete with the rest of the host, and `miren logs system` notes the restart on the next boot. Apps and addon databases run outside the limit and keep running through a restart. Existing installs pick it up on their next upgrade. If you set a memory limit by hand in `/etc/systemd/system/miren.service`, move it to `sudo systemctl edit miren`, or the managed limit will override it. See [Troubleshooting](./troubleshooting.md#the-server-restarted-on-its-own). ([#1210](https://github.com/mirendev/runtime/pull/1210))
- **`miren deploy` output you can script against** - Piped deploy output was full of escape codes and replayed every build step, and the version was buried in a sentence. Non-terminal output is now plain lines ending in `Version: <id>`. `--format json` prints one result document, `--format jsonl` streams the whole deploy as events, and `--quiet` drops the progress. ([#1194](https://github.com/mirendev/runtime/pull/1194))
- **Deploys wait for their addons** - A deploy used to move on while its addon was still provisioning, so deploy tasks ran without `DATABASE_URL` and the health check timed out. Deploys now wait for each addon to become active and fail with the reason if it errors. `miren addon list` gains a STATUS column. ([#1190](https://github.com/mirendev/runtime/pull/1190))
- **Know which versions you're running** - `miren version --server` shows CLI and server side by side, and `miren doctor` warns when either is behind the latest release, with the command to fix it. The `doctor config|server|auth` subcommands are gone; plain `miren doctor` covers them. ([#1212](https://github.com/mirendev/runtime/pull/1212), [#1244](https://github.com/mirendev/runtime/pull/1244))
- **App usage over any window** - `miren top --apps` only counted sandboxes alive right now, so last week's usage for an app redeployed Tuesday only covered since Tuesday. It now reads the metrics store, which keeps a month of samples. `--series` adds a time series and `--contributors` breaks the total down per sandbox, including ones that are gone. ([#1215](https://github.com/mirendev/runtime/pull/1215))
- **Runtime health metrics ship with your app metrics** - If you send managed app metrics to a Remote Write destination, Miren's own operational metrics (control process memory and goroutines, etcd health, host usage, restarts, build version) now go with them. No new config. ([#1211](https://github.com/mirendev/runtime/pull/1211), [#1250](https://github.com/mirendev/runtime/pull/1250))
- **Upgrades bring containerd and runc along** - Upgrades kept only the `miren` binary from the release bundle, so a host stayed on the containerd and runc it was installed with. They now install the whole bundle, starting with the upgrade after this one. ([#1237](https://github.com/mirendev/runtime/pull/1237))
- **Image pull failures show up in `miren logs`** - A node that couldn't pull an image left a crash-looped deploy with no logs, and the reason only in that node's journal. It now shows up in `miren logs` and the failed deploy's output. ([#1196](https://github.com/mirendev/runtime/pull/1196))
- **Typos get a suggestion** - An unknown command or flag now names the word at fault and suggests the closest match, instead of `unexpected arguments: [lst]`. ([#1209](https://github.com/mirendev/runtime/pull/1209))
- **Deleted apps' images get cleaned up** - Images from deleted apps were never garbage collected. They're now reclaimed after 24 hours, including ones already on existing servers. ([#1224](https://github.com/mirendev/runtime/pull/1224))
- **Deployment history is bounded** - Each app now keeps its last 25 deployment records plus anything from the past 30 days, configurable under [`[deployment]`](./server-config.md#deployment). Registered clusters only prune what Miren Cloud already holds. ([#1230](https://github.com/mirendev/runtime/pull/1230))
- **Container installs match the CLI's version** - `miren server container install` defaulted to the `:latest` image. It now installs the CLI's own version, and `--version` picks another. ([#1195](https://github.com/mirendev/runtime/pull/1195))
- **The Linux binary is fully static** - It now runs on NixOS, Alpine, and other musl hosts. ([#1223](https://github.com/mirendev/runtime/pull/1223))
- **Deploys from jj workspaces record their commit** - Deploys from a jj repo without a colocated `.git` recorded no commit, branch, or author, and shipped `.jj` in the build context. Both fixed. ([#1213](https://github.com/mirendev/runtime/pull/1213))

**Security**
- **Ingress auth cookies keep their `Secure` flag** - Concurrent http and https requests through ingress auth could flip each other's `Secure` flag, and a client could strip it by sending `X-Forwarded-Proto: http` over TLS. Forwarded-scheme headers are now trusted only in `behind-proxy-http` mode. Cookie contents were encrypted throughout. ([#1245](https://github.com/mirendev/runtime/pull/1245), [#1247](https://github.com/mirendev/runtime/pull/1247))

**Bug Fixes**
- **`miren app run` returns when the command finishes** - Attached runs used to hang for up to a minute after the command exited. ([#1206](https://github.com/mirendev/runtime/pull/1206))
- **Services with UDP ports become healthy** - A service declaring a UDP port was killed on every deploy because the boot check only saw TCP listeners. Thanks [@veganstraightedge](https://github.com/veganstraightedge)! ([#1254](https://github.com/mirendev/runtime/pull/1254))
- **`miren env set` keeps commas in values** - `-e KEY=a,b,c` set three variables instead of one. It also accepts bare `KEY=VALUE` arguments now. ([#1197](https://github.com/mirendev/runtime/pull/1197))
- **Uploads survive a file growing mid-deploy** - A file being written during the upload, like a log or the deploy's own redirected output, failed the deploy with `archive/tar: write too long`. ([#1194](https://github.com/mirendev/runtime/pull/1194))
- **Downloaded CLIs report their real version** - Standalone CLI binaries since v0.14.0 reported `HEAD:<sha>` as their version. ([#1207](https://github.com/mirendev/runtime/pull/1207))
- **Long-lived sandboxes recover workload identity** - A sandbox that lost its token registration got `403 invalid token` until restarted. It now recovers on its next retry. ([#1255](https://github.com/mirendev/runtime/pull/1255))
- **The server starts after a crash mid-shutdown** - A leftover containerd task record could stop etcd or buildkit from starting on every later boot. ([#1228](https://github.com/mirendev/runtime/pull/1228))
- **Old `/usr/local/bin/miren` launchers get repaired** - A host whose launcher predated the managed symlink kept an old CLI through every upgrade. Startup now fixes the link. ([#1246](https://github.com/mirendev/runtime/pull/1246))
- **Build traces reach HTTP-only OTLP collectors** - With `OTEL_EXPORTER_OTLP_ENDPOINT` set, buildkit tried gRPC and every build's spans were dropped. ([#1248](https://github.com/mirendev/runtime/pull/1248))
- **`miren server install` works without `HOME`** - Under cloud-init it finished the install but failed to write the local client config. ([#1252](https://github.com/mirendev/runtime/pull/1252))
...
**Security**
- **`miren sandbox list --json` no longer prints resolved secrets** - It printed raw sandbox entities, which include the resolved container environment. Listing now goes through its own RPC with an allowlisted response, and both table and JSON output use it. A new CLI falls back to raw entities only when it can tell the server is too old to have the new call. ([#1154](https://github.com/mirendev/runtime/pull/1154))

**Features**
- **See what's using your cluster's CPU and memory** - Diagnosing a busy cluster meant listing sandboxes, then dialing runners one at a time for a raw dump of lifetime counters, with nothing aggregated and no way to tell an app's load from Miren's own. `miren top` lists sandboxes busiest-first, `--apps` totals each app across everything it owns, and `--nodes` shows which machine is hot and how much of that is platform overhead. Every runner already ships its metrics to one store, so the view covers the whole cluster and can look back in time: `--since 1h --aggregate max` finds the spike that paged you an hour ago. `miren sandbox inspect` picks up from there with what one sandbox runs, how it exited, and whether its pool has stopped replacing it. ([#1153](https://github.com/mirendev/runtime/pull/1153))
- **Route a hostname to a service other than `web`** - An HTTP route always reached the app's `web` service, so an app with a separately named API or dashboard service had no way to put a hostname in front of it. `miren route set <host> <app> --service api` now selects any HTTP-capable named service; leaving the flag off still means `web`. ([#1095](https://github.com/mirendev/runtime/pull/1095))
- **Unattended cluster enrollment** - Registering a cluster meant a human approving in a browser, which doesn't work for a machine booting itself from cloud-init. `miren server install --enroll-token <token>` takes a single-use token from Miren Cloud and saves an approved registration in one shot. A token cloud rejects is terminal on purpose, so an unattended machine doesn't sit waiting for an approval nobody knows to give. ([#1102](https://github.com/mirendev/runtime/pull/1102))
- **Scriptable `miren cluster add`** - Scripting `miren cluster add` meant passing `--address` yourself, which skipped the useful part: trying each address the cluster advertises, asking Miren Cloud to relay when none answer, and saving the cluster's certificate to check later connections against. `--cluster prod` on its own now does all of that, `miren cluster available` lists the names, and `--format json` reports the outcome with a code you can branch on. ([#1118](https://github.com/mirendev/runtime/pull/1118))
- **Managed application metrics** - A service can opt into managed metrics without exposing its endpoint publicly. The coordinator finds each running sandbox, scrapes it privately, and sends samples to your Prometheus Remote Write destination with Miren's own app, version, service, sandbox, runner, and cluster labels attached. Scrape and delivery failures show up in `miren logs system vmagent`. ([#1087](https://github.com/mirendev/runtime/pull/1087))

**Improvements**
- **Less config when bringing your own image** - Running software someone else packaged meant repeating what the image already knew: a wrapper Dockerfile to clear an entrypoint, a `command` to put it back, a `port` the image already declared. `services.web.image` is now a complete build source on its own, inheriting the image's entrypoint, working directory, and single exposed TCP port, and pinning the deploy by digest. Set `args` to keep the image's `ENTRYPOINT` with a different `CMD`, and `app status` names where a version came from. Explicit config still wins. ([#1090](https://github.com/mirendev/runtime/pull/1090), [#1099](https://github.com/mirendev/runtime/pull/1099), [#1108](https://github.com/mirendev/runtime/pull/1108), [#1110](https://github.com/mirendev/runtime/pull/1110), [#1106](https://github.com/mirendev/runtime/pull/1106))
- **Builds keep running when the CLI goes away** - Cancellation rode on the CLI's RPC connection, so closing a terminal stopped the build, and `miren deploy cancel` worked only because the initiating CLI noticed and cancelled locally. The server-owned deployment record is the control plane now, so a build outlives the CLI that started it, and Ctrl-C writes an explicit cancellation that unwinds cleanly. ([#1112](https://github.com/mirendev/runtime/pull/1112), [#1094](https://github.com/mirendev/runtime/pull/1094))
- **One deployment shows as active** - Legacy rows kept an `active` status after another deploy took over, so `miren app history` could show several active deployments at once even though the app served one. The app's serving pointer decides now; everything else reads `succeeded`. ([#1111](https://github.com/mirendev/runtime/pull/1111))
- **`miren app runs` is easier to scan** - Runs now show short ids the way `app history` and `app status` already do, and sort by start time, newest first. The old sort pulled failures to the front, so the position of the run you just triggered depended on how unrelated older runs ended. `--format json` still emits full ids. ([#1092](https://github.com/mirendev/runtime/pull/1092))
- **Finished sagas no longer hold up startup** - A saga is saved on every status change, and the entity index kept the old value each time, so an execution stayed listed under every status it had passed through. Recovery then read the whole set to find the few it owned, which on a 4 GB host could stall startup before ingress came up. The backlog is now swept in the background, and recovery and retention read a page at a time. ([#1166](https://github.com/mirendev/runtime/pull/1166), [#1167](https://github.com/mirendev/runtime/pull/1167))
- **Builds resolve DNS through Miren's own resolver** - v0.14 stopped the builder hardcoding public DNS, but a host using the systemd-resolved stub (`nameserver 127.0.0.53`) still ended up on `8.8.8.8`, because BuildKit strips loopback nameservers and substitutes its own. Builds now point at Miren's bridge resolver, which already serves sandboxes and forwards to the host's real upstream. ([#1103](https://github.com/mirendev/runtime/pull/1103))
- **Sandbox DNS failures say what happened** - A failed lookup returned a bare SERVFAIL, and glibc could spend ten seconds waiting for it. Resolvers now work within one 1.5-second budget for the whole request and log the query, client, transport, and upstream that failed. The bridge resolver serves UDP and TCP on both IPv4 and IPv6, and retries a truncated reply over TCP. ([#1115](https://github.com/mirendev/runtime/pull/1115))
- **Server components start in parallel when they can** - Startup was one long ordered function, with dependencies expressed through line placement and sleeps. Coordinators and runners now start independent components at the same time and wait only where something genuinely depends on something else, with shutdown following the same order in reverse. Distributed runners work the same way. ([#1100](https://github.com/mirendev/runtime/pull/1100), [#1158](https://github.com/mirendev/runtime/pull/1158), [#1161](https://github.com/mirendev/runtime/pull/1161))

**Bug Fixes**
- **BuildKit restarts cleanly** - Stopping Miren while BuildKit was still shutting down could leave containerd's task behind, and the next boot would fail with `task miren-buildkit: already exists`. Evicting that task is a precondition for startup now, and cleanup waits for the process to exit before deleting it. ([#1148](https://github.com/mirendev/runtime/pull/1148))
- **Idle apps stay idle** - The deployment launcher treated a reused pool at zero instances as a fresh deploy, so the minutely resync kept cold-starting scale-to-zero autoscale apps and waiting for them to become ready. Scale-to-zero pools now stay drained unless the pool is actually moving to a new version. ([#1150](https://github.com/mirendev/runtime/pull/1150))
- **Deploys don't wait on a removed node** - A disk-aware deploy could stall when a historical, diskless sandbox stayed STOPPED on a node that no longer existed, because each resync rewrote the decommissioned pool and waited out the same timeout. Terminal sandboxes whose node is gone are now collected, so old records stop holding pools open. ([#1162](https://github.com/mirendev/runtime/pull/1162))
- **Routine client disconnects stop printing stack dumps** - The reverse proxy uses an abort signal as ordinary control flow when a response stream can't continue, and httpingress logged every recovered panic before passing it back, so a client hanging up mid-response appeared as an Error-level stack dump. ([#1138](https://github.com/mirendev/runtime/pull/1138))
- **Requests to addon-backed services stop timing out** - An app with a `[addons.miren-postgresql]` addon could deploy and report healthy while every request touching the database hung at the ingress. On kernels built without `CONFIG_NFT_OBJREF`, the named traffic counter in every service chain was rejected, which threw out the whole batch of rules, and our cache then recorded rules the kernel never took, so the next pass saw nothing to do. Service chains now use an anonymous counter, which works on every kernel, and a rejected batch clears the cache. One visible change: the counter is per service and resets on the periodic rebuild, so it reads as a rolling window rather than a running total. ([#1165](https://github.com/mirendev/runtime/pull/1165), [#1180](https://github.com/mirendev/runtime/pull/1180))
- **HTTP metrics no longer grow without bound** - The ingress kept a counter for every distinct URL path it ever served and never evicted one, so an app with IDs in its routes, or a client walking random URLs, grew the server's memory and its metrics series indefinitely. In one v0.14.0 profile those counters were most of the live heap. Paths are now capped at 100 tracked entries per node, with the rest counted under `other`, and the busiest paths earn their slots over time rather than first-come. `miren app status` reads its top-paths view from those counters directly, so per-path error rates and latencies stay consistent with each other. ([#1188](https://github.com/mirendev/runtime/pull/1188))
- **Stuck saga records from older versions get cleared** - Some of the runtime's internal workflow records (sagas) written by older versions could never finish and never be deleted: a retry finds its earlier attempt by name, but one with a generated name has nobody left who can name it, and retention only removes finished work. Each one stayed in the store forever and every recovery pass paid to read it; one pre-upgrade snapshot held nearly 20,000 for a single addon. A background sweep now marks anything untouched for seven days as failed, which lets the existing rules retry or reap it. Setting `saga.retention_period` to 0 still means leave saga records alone, and now stops this sweep too. ([#1185](https://github.com/mirendev/runtime/pull/1185))
- **Controllers don't drop or replay stale work under load** - A busy controller queued whole event snapshots in a bounded channel, so it could act on stale state, drop an event, or sit waiting for another change after a transient failure. Each entity now gets one queue entry, is re-read immediately before reconciling, and retries on its own bounded schedule. ([#1164](https://github.com/mirendev/runtime/pull/1164))
- **A slow-starting image isn't torn down once its port is up** - The port wait raced its success signal against its deadline, so a sandbox could occasionally be marked dead with `nothing is listening on :N` just as the port came up. The wait re-checks before reporting a timeout. The next pool retry usually recovered, so this showed up as an intermittent boot failure. ([#1133](https://github.com/mirendev/runtime/pull/1133))
- **`miren login` waits out a transient outage** - The device-login poll treated any non-JSON response from the token endpoint as fatal, so a proxy returning a 502 HTML page during a brief outage ended the login with `failed to parse response: invalid character '<'`. A non-200 response keeps polling within the existing ten-minute window; a 200 with a non-JSON body is still an error. ([#1136](https://github.com/mirendev/runtime/pull/1136))
- **A failed disk restore doesn't leave a volume behind** - Cleanup deleted the disk entity directly, which skips the path that reaps its volume, so a restore that failed partway could leave the volume behind along with its mount and, where cloud is configured, a volume on the cloud side. Restore now orders its writes so a failure leaves nothing, and cleanup goes through the normal deletion path. ([#1132](https://github.com/mirendev/runtime/pull/1132))
- **Streaming RPCs reconnect after a server disappears** - A watch could stop reconnecting for the life of the process when the remote went away mid-handshake: the caller's cancellation never reached the wait for HTTP/3 settings, so the retry loop had nothing to retry on. Both phases of the dial are bounded by the caller's context now. ([#1119](https://github.com/mirendev/runtime/pull/1119), [#1143](https://github.com/mirendev/runtime/pull/1143))
...
**Features**
- **Secrets** - Store credentials in a built-in cluster backend with `miren secret`, then reference them from `app.toml` or `miren env set --backend cluster --ref db/url`. Values are encrypted at rest and materialized into the container without being written to config or etcd. References pin to a version so rollbacks restore the old value. Dockerfile builds can also consume declared `[[build.secrets]]` through BuildKit secret mounts, keeping them out of image layers and logs. See the [secrets guide](./secrets.md). ([#1027](https://github.com/mirendev/runtime/pull/1027), [#1028](https://github.com/mirendev/runtime/pull/1028), [#1031](https://github.com/mirendev/runtime/pull/1031), [#1078](https://github.com/mirendev/runtime/pull/1078))
- **App Tasks** - Declare an entry under `tasks` like `[tasks.migrate]` to create a command and trigger to run work on deploy, on a systemd-calendar schedule, or by hand. Deploy tasks gate the version flip on success. Runs keep durable exit codes and logs, and `miren app runs` lets you inspect or reattach to them. Task-only apps can set `web = false`. See the [tasks guide](./tasks.md). ([#1019](https://github.com/mirendev/runtime/pull/1019), [#1043](https://github.com/mirendev/runtime/pull/1043), [#1045](https://github.com/mirendev/runtime/pull/1045), [#1080](https://github.com/mirendev/runtime/pull/1080))
- **Call the cluster API from inside your app** - Sandboxed code can now call the cluster API using its workload identity. `miren` works inside a container with no setup, under an app-scoped read-only role by default. Set `workload_role` in `app.toml` for another app role, or use `m app set-workload-role` for a cluster-wide role. See the [in-cluster API guide](./in-cluster-api.md). ([#1018](https://github.com/mirendev/runtime/pull/1018))
- **SQLite as an addon** - Add `[addons.miren-sqlite]` to give an app a SQLite database in its own container, replicated to the coordinator and restored when the app moves to a new node. Apps read `DATABASE_URL`, and Miren requires each service using the single-writer database to run one fixed instance. This one's early, so kick the tires and tell us what breaks. ([#1051](https://github.com/mirendev/runtime/pull/1051))
- **Per-route request timeouts** - One slow route used to be everyone's problem: to keep a long-poll or a streaming endpoint from getting cut off, you had to raise the cluster-wide request timeout for every app. Now a route carries its own: `miren route timeout longpoll.example.com 10m`, or `--clear` to fall back to the cluster default. ([#1057](https://github.com/mirendev/runtime/pull/1057))
- **Maintenance mode for routes** - `miren route down app.example.com --reason "Upgrading the database"` takes a hostname out of service and shows visitors a holding page. It returns a 503 with an optional `Retry-After`, while the app keeps running for commands and one-shot migrations. `miren route up` restores traffic. See the [maintenance mode guide](./maintenance-mode.md). ([#1066](https://github.com/mirendev/runtime/pull/1066))
- **Reach a cluster behind NAT through Miren Cloud** - A cluster behind NAT or on a home network — anything without a public route — was awkward to run `miren` against. Now you can reach it over the authenticated connection it already holds up to Miren Cloud: `miren cluster add --via-cloud`. Your credentials ride end-to-end inside the request; cloud only relays the bytes and can't read them. ([#979](https://github.com/mirendev/runtime/pull/979), [#1056](https://github.com/mirendev/runtime/pull/1056))
- **Back up disks to Miren Cloud** - Disks with `provider = "miren"` can now back up to Miren Cloud. Universal-mode disks take an on-demand snapshot with `miren disk backup --cloud` (not automatic — reading a live-mounted disk isn't crash-consistent, so you pick the moment). Accelerator-mode continuous backup, which had a bug that kept it from ever working, now works. ([#1049](https://github.com/mirendev/runtime/pull/1049))
- **Disconnect a cluster from Miren Cloud** - `miren server unregister` is the clean inverse of `register`: it revokes the cluster's cloud credentials, clears the local registration, and restarts. Pass `--local-only` to forget it locally without contacting cloud. ([#1021](https://github.com/mirendev/runtime/pull/1021))
- **Move where workload identity is anchored** - New clusters now anchor workload-identity token issuance at miren.cloud by default — cloud serves the public discovery endpoint on the cluster's behalf, while the signing key never leaves your box. Existing clusters move with `miren server identity-anchor cloud` (or `cluster` to go back), and tokens issued mid-switch keep working through an overlap window. ([#1052](https://github.com/mirendev/runtime/pull/1052))

**Improvements**
- **Builds and sandbox startup survive a restart** - Both now use the saga engine by default, journaling each step so a control-plane restart can resume or unwind work instead of stranding it. The previous path remains as an escape hatch for one release: start the server with `--labs -sagas` or `MIREN_LABS=-sagas`. See [Miren Labs](./labs.md). ([#972](https://github.com/mirendev/runtime/pull/972))
- **Next.js builds** - Deploy a Next.js app with no Dockerfile and Miren now detects it, runs `next build`, and serves it with `next start`. Before, an auto-built Next.js app shipped without its compiled output and crashed on boot. ([#980](https://github.com/mirendev/runtime/pull/980))
- **app.toml editor support** - Miren now publishes a JSON Schema for app.toml, so an editor that understands it can validate your config and autocomplete fields as you type. Linked from the app.toml reference. ([#1070](https://github.com/mirendev/runtime/pull/1070), [#1075](https://github.com/mirendev/runtime/pull/1075))
- **`sandbox exec` finds the sandbox for you** - `miren sandbox exec --app myapp -- <cmd>` picks a running sandbox instead of making you list them, find a RUNNING row, and copy an ID. `sandbox list` takes the same `--app`/`--service` filters. ([#1058](https://github.com/mirendev/runtime/pull/1058))
- **Clearer debug output** - `miren debug entity` was rebuilt around readable cards and tables and no longer chokes on large kinds. `miren debug saga` lists saga executions and shows each one's timing, errors, and undo state without decoding base64 by hand. `miren debug etcdctl` talks to the embedded etcd with the endpoint and credentials filled in for you. ([#1030](https://github.com/mirendev/runtime/pull/1030), [#1026](https://github.com/mirendev/runtime/pull/1026), [#1023](https://github.com/mirendev/runtime/pull/1023))
- **A warning when local disks share a directory** - Two `provider = "local"` disks at different mount paths silently pointed at the same host directory. Miren now warns at deploy time and names the paths, before the collision corrupts data. ([#1065](https://github.com/mirendev/runtime/pull/1065), [#1084](https://github.com/mirendev/runtime/pull/1084))
- **`server install` checks its dependencies** - `miren server install` and `miren runner install` now verify `iptables` and `nft` are present and tell you how to install them for your distro, instead of failing quietly at runtime when networking doesn't come up. ([#1067](https://github.com/mirendev/runtime/pull/1067))
- **Finished sagas get cleaned up** - Now that sagas run by default, terminal executions no longer pile up forever — they're swept after 7 days, tunable with `saga.retention_period`. ([#1022](https://github.com/mirendev/runtime/pull/1022))

**Bug Fixes**
- **Addon variables survive rollback and restart** - An addon's variables (like `DATABASE_URL`) were copied into a config version once and never updated, so deploying, rolling back, or restarting an older version handed the app a config with the variable missing — recoverable only by destroying the addon, which dropped the database. Bindings now resolve live, so the variable is always there. ([#1063](https://github.com/mirendev/runtime/pull/1063))
- **Addons recover after a coordinator crash** - Killing a coordinator partway through provisioning an addon left it stuck "provisioning" forever, with the app up but never actually running. Recovery now finishes the job. ([#1048](https://github.com/mirendev/runtime/pull/1048))
- **Disk handoff waits for the old process to let go** - On a disk-backed deploy, a replacement could start against the same local disk while the retiring process still held it — once observed deleting a file its replacement had just written. Miren now waits for the old sandboxes to fully exit first. ([#1077](https://github.com/mirendev/runtime/pull/1077))
- **Rolling back to a specific version works again** - `miren app rollback` to a canonical version ID came back "missing" because the lookup mangled the ID. It now accepts bare, short, and full references. ([#1076](https://github.com/mirendev/runtime/pull/1076))
- **Auto-builds keep the base image's environment** - An app built without a Dockerfile lost every environment variable its base image set, which broke Bun (its recent images move `node` onto a `PATH` the base image sets). Auto-builds now inherit that environment. ([#1079](https://github.com/mirendev/runtime/pull/1079))
- **Builds respect your network's DNS** - The builder hardcoded public DNS servers, so builds on a network with an internal resolver — split-horizon DNS, a private mirror — couldn't resolve internal names. It now uses the host's DNS configuration. ([#1083](https://github.com/mirendev/runtime/pull/1083))
- **Wildcard routes fall back to the active version** - A wildcard route (`*.app.example.com`) read the subdomain as a version label and returned 404 when it wasn't one, so it couldn't serve tenants off your active version. Only wildcard routes now fall back; exact routes stay strict. ([#1068](https://github.com/mirendev/runtime/pull/1068))
- **`cluster add` writes a config that connects** - Adding a cluster by hand wrote an entry with no identity, so the next command failed with a `tls: failed to find any PEM data` error far from the real cause. It now records the credentials from your login, and `cluster remove` can remove the active cluster. ([#1042](https://github.com/mirendev/runtime/pull/1042))
- **Accelerator disk backups keep running** - An accelerator-mode disk stopped backing up the moment it acquired its cloud lease, because of a mismatched volume ID, then retried the rejected upload forever. Fixed. ([#1074](https://github.com/mirendev/runtime/pull/1074))
- **`miren logs` exits without a scary error** - A finished `miren logs` run could print all its records, succeed, and still leave a `stream reset` error on the terminal from a harmless shutdown race. That's gone. ([#1050](https://github.com/mirendev/runtime/pull/1050))
- **Connect notice stays out of piped output** - The "connecting to…" notice went to stderr even when you were piping `--format json 2>&1` into another tool, corrupting the stream. It's now suppressed when the output isn't a terminal. ([#1053](https://github.com/mirendev/runtime/pull/1053))
- **Networking recovers after an etcd compaction** - A per-node lease watch could get stuck after etcd compacted its history, spamming logs and quietly disabling lease-revocation detection until the process restarted. Picked up the upstream flannel fix. ([#1035](https://github.com/mirendev/runtime/pull/1035))

**Documentation**
- **Minimum working examples on every feature page** - Every feature page now opens with a copy-pasteable minimal example, so you can get something working before reading the reference. ([#1046](https://github.com/mirendev/runtime/pull/1046))
- **Raw Markdown for docs pages** - Every docs page is now served as raw Markdown at its `.md` URL, friendlier for agents and LLMs reading the docs. ([#1071](https://github.com/mirendev/runtime/pull/1071))
- **Self-hosting headscale** - A new recipe walks through running headscale (an open-source Tailscale control server) on Miren. ([#1054](https://github.com/mirendev/runtime/pull/1054))
...
**Features**
- **Distributed runners** - One machine eventually runs out of room, so now you can add another: `miren runner install` joins a node to your coordinator and Miren schedules sandboxes across the cluster, with node-to-node traffic on an encrypted WireGuard overlay. On by default after baking in labs, with `MIREN_LABS=-distributedrunners` as the escape hatch for one release. See the [distributed runners guide](./distributed-runners.md). ([#971](https://github.com/mirendev/runtime/pull/971), [#983](https://github.com/mirendev/runtime/pull/983), [#985](https://github.com/mirendev/runtime/pull/985), [#987](https://github.com/mirendev/runtime/pull/987), [#989](https://github.com/mirendev/runtime/pull/989), [#992](https://github.com/mirendev/runtime/pull/992), [#1008](https://github.com/mirendev/runtime/pull/1008))
- **Running Miren on a Tailscale-only host** - A tailnet-only cluster failed every connection with a bare timeout, because Miren dropped tailnet addresses from what a cluster advertises and sent QUIC handshakes too large for a 1280-MTU link. Both are fixed, and there's a [guide for running on a tailnet](./tailscale.md). ([#1000](https://github.com/mirendev/runtime/pull/1000), [#1001](https://github.com/mirendev/runtime/pull/1001))

**Security**
- **Safer archive extraction for build contexts and releases** - Build contexts reach the extractor before BuildKit can isolate them, and it trusted every archive entry name. Extraction and the release downloader now check containment and refuse to write through existing symlinks. ([#988](https://github.com/mirendev/runtime/pull/988), [#1005](https://github.com/mirendev/runtime/pull/1005))
- **Password-protected routes reject off-site redirects** - The return-path check rejected `//evil.com` but let `/\evil.com` through, which browsers normalize to the same thing and follow off-site. ([#1006](https://github.com/mirendev/runtime/pull/1006))
- **Workload identity subjects are unambiguous** - Subject claims joined labels and values with `:` while still allowing that character inside the values, so two distinct workloads could collapse to one federation identity. ([#1010](https://github.com/mirendev/runtime/pull/1010))

**Improvements**
- **Deployments are tracked by the server** - The CLI used to own the deployment record, so a client that crashed could strand one, and the lock against concurrent deploys was a list-then-create with a 30-minute timeout. The servers own it now, behind a real state machine and a lock two racing deploys cannot both win. ([#975](https://github.com/mirendev/runtime/pull/975))
- **Clearer errors when a cluster is unreachable** - `timeout: no recent network activity` was the answer to nearly every problem, and it usually just meant the server wasn't running. Miren tells "not running", "too old to answer this", and "healthy but doesn't have what you asked for" apart now, and `miren doctor` gives one verdict instead of separate probes that disagree. ([#996](https://github.com/mirendev/runtime/pull/996))
- **Coordinators and runners log less** - Every systemd-installed coordinator ran at Debug indefinitely because `server install` defaulted to `-vv`, and the audit trail spent about half its bytes re-recording unchanged certificate details on every RPC. Daemons start at Info now and those records are deduplicated, cutting a coordinator's output by roughly two thirds. ([#1009](https://github.com/mirendev/runtime/pull/1009))
- **Labs features report their status at startup** - A feature that defaults to on used to say nothing at boot, which is the state an operator most wants confirmed. Every boot now logs where each labs feature ended up, and [Miren Labs](./labs.md) documents the `-` prefix (`MIREN_LABS=-distributedrunners`) that turns one back off. ([#1012](https://github.com/mirendev/runtime/pull/1012))

**Bug Fixes**
- **Deploys keep serving traffic for apps with mixed services** - The shim that keeps legacy local storage mounted probed at the app level but injected a disk into every service, so a stateless web service sitting beside a Valkey service took the drain-then-create deploy path and left a few seconds with nothing listening. ([#997](https://github.com/mirendev/runtime/pull/997))
- **Sandboxes on reused addresses receive workload identity tokens** - The token server maps a caller's source address to an app and never corrected a stale entry, so a sandbox that landed on a recently recycled address got `403 invalid token` for its entire life. ([#1003](https://github.com/mirendev/runtime/pull/1003))
- **Sandbox metrics continue across a restart** - Sandboxes that outlive a restart get adopted, but their cgroup metrics were never re-registered, so CPU and memory stopped at the restart while logs kept flowing and everything looked healthy. ([#999](https://github.com/mirendev/runtime/pull/999))
- **Schema reindexing completes on large clusters** - The reindex was capped by a two-minute startup deadline and restarted from the first entity on every boot, so a large cluster never finished one and entities could stay missing their index entries. It's a resumable background job now. ([#1004](https://github.com/mirendev/runtime/pull/1004))
- **GitHub autodeploy works for renamed and newly created repositories** - GitHub now bakes numeric IDs into the OIDC subject claim, and `miren auth ci add --github` built its match pattern from the owner and repo names, so bindings were inert from the moment you created them. They match on the `repository` claims now. Upgrade servers before CLIs. ([#995](https://github.com/mirendev/runtime/pull/995))
- **Sandbox tokens are released when the sandbox stops** - Token state was released on entity delete, which the normal exit path never reaches, so the refresher kept minting a fresh JWT every 45 minutes for sandboxes that had exited long ago. ([#1002](https://github.com/mirendev/runtime/pull/1002))
- **Disk undelete waits for the volume to mount** - `miren disk undelete` reported the disk provisioned before the runner had mounted the volume, so a lease created in that window stayed failed forever. ([#970](https://github.com/mirendev/runtime/pull/970))
...
**Features**
- **Miren Anywhere** - Deploy from your laptop, a homelab box, or anything behind NAT or a firewall, and your apps are reachable anyway. Your cluster dials out to Miren Cloud and a POP at the edge carries inbound traffic back to it, so nothing ever has to accept a connection from the internet, and the "deploy succeeds but the URL just times out" wall disappears. Read the [launch post](https://miren.dev/blog/miren-anywhere). ([#913](https://github.com/mirendev/runtime/pull/913), [#924](https://github.com/mirendev/runtime/pull/924), [#778](https://github.com/mirendev/runtime/pull/778))

**Improvements**
- **Env vars injected into sandboxes gained a `MIREN_RUNTIME_` prefix** - `MIREN_APP` meant two different things: the app name injected into your running sandbox, and the CLI's target app read from your own environment. Running `miren` inside a sandbox therefore picked up the sandbox's app and silently ignored the `.miren/app.toml` in front of it. The injected variables now have unambiguous namespaced names: `MIREN_RUNTIME_APP`, `MIREN_RUNTIME_VERSION`, `MIREN_RUNTIME_INSTANCE_NUM`. The old names (`MIREN_APP`, `MIREN_VERSION`, `MIREN_INSTANCE_NUM`) are still injected as deprecated aliases with the same values, so apps reading them keep working — but they will be removed in a future release, so migrate to the `MIREN_RUNTIME_*` names when convenient. `MIREN_APP` as a CLI input is unchanged (CI pipelines that set it keep working), and running `miren` inside a sandbox now resolves the app from `.miren/app.toml` rather than the injected alias. The `MIREN_IDENTITY_*` workload identity variables are unchanged. See [App Configuration](./app-configuration.md#variables-miren-injects). ([#925](https://github.com/mirendev/runtime/pull/925), [#940](https://github.com/mirendev/runtime/pull/940))
- **Version retention reclaims disk under pressure** - Every retained app version pins a container image, an overlayfs snapshot, and a registry blob, so on a frequently-deploying cluster the age-based retention window could hold enough recent versions to fill the disk faster than they age out. When storage crosses the image-GC threshold (80%), version retention now sheds that age-based surplus and prunes each app down to its active version plus the retention count, reclaiming space automatically. Below the threshold nothing changes, and the active version and retention count are always kept. ([#936](https://github.com/mirendev/runtime/pull/936))
- **`miren deploy --summary-json`** - Write a deploy's results (the deploy id, the version Miren assigned, and the route URLs) to a JSON file, so CI and other automation can read them directly instead of scraping deploy output. The [`mirendev/actions` deploy action](https://github.com/mirendev/actions/pull/3) uses it to surface those details in CI deploy output. ([#906](https://github.com/mirendev/runtime/pull/906))
- **Audit logging for control-plane access** - Miren now records an audit trail of RPC calls against the control plane (who called what, from where, and the outcome) plus every successful certificate authentication, tagged `module=audit` so it's easy to filter in the server log. ([#910](https://github.com/mirendev/runtime/pull/910))
- **etcd sizes itself to the node** - The bundled etcd now scales its memory footprint (roughly 10% of system RAM) and its compaction and snapshot cadence to the machine at startup, instead of using one fixed configuration everywhere, so it stays a well-behaved co-tenant on everything from a 1 GB box to a 64 GB one. Its keyspace quota keeps a safe floor and only widens on larger nodes, and values stay overridable via `ETCD_*` env. ([#900](https://github.com/mirendev/runtime/pull/900))
- **A clearer answer when a self-hosted cluster is unreachable** - A cluster with a firewalled inbound port used to register fine but silently never appear in `miren cluster add`, which looked like an auth problem. Those clusters now show up marked "no reachable address" with a pointer to the fix, and the dashboard can spell out which port and protocol were blocked. ([#896](https://github.com/mirendev/runtime/pull/896), [#916](https://github.com/mirendev/runtime/pull/916))
- **`miren logs` is color-coded by service** - App output printed at the same weight as the router's request firehose, so your own logs were hard to pick out. Each line now gets a stable per-service color and a `service.id` prefix you can follow through interleaved output, and the router gets its own lane with the HTTP status colored by class so a `500` pops out of a wall of `200`s. Color and spacing adapt to the terminal but the content doesn't, so piping to `grep` is unchanged; run `miren debug colors` to see the palette. ([#942](https://github.com/mirendev/runtime/pull/942))

**Bug Fixes**
- **`miren app restart` now actually restarts** - `miren app restart` cheerfully reported "stopped 0 sandboxes" and then, well, did nothing. A typo in an entity-system write had the stop silently rejected; we corrected it and tightened up validation so the same class of bug can't recur. ([#927](https://github.com/mirendev/runtime/pull/927), [#932](https://github.com/mirendev/runtime/pull/932))
- **Mounted disks are writable by your app's user** - A disk mounted as root-owned, so an image running as a non-root user couldn't write to it without a `chown` shim in the entrypoint. Miren now hands the disk to whatever user your container runs as, and an `owner` field (`keep`, `uid`, or `uid:gid`) covers the cases that want something different. ([#935](https://github.com/mirendev/runtime/pull/935))
- **Sensitive env-var values are masked in `miren app status`** - `miren app status` didn't properly mask sensitive env-var values, so a `DATABASE_URL` with an embedded password would print in full. It now masks them consistently, and you can reveal a value one at a time with `miren env get <key> --unmask`. ([#911](https://github.com/mirendev/runtime/pull/911))
- **Entity-store index leaks plugged** - We chased down a couple of spots where the entity store's index management left stale entries behind. Those leaks are plugged, and a new background GC keeps any future leak from growing the store unbounded. ([#898](https://github.com/mirendev/runtime/pull/898), [#903](https://github.com/mirendev/runtime/pull/903), [#917](https://github.com/mirendev/runtime/pull/917))
- **Superseded app instances are retired after an update** - A runtime update that changed how sandboxes are built could leave a stateless app running a duplicate set of instances alongside the originals. Miren now retires the superseded instances once their replacements are healthy. ([#934](https://github.com/mirendev/runtime/pull/934))
- **Orphaned disk mounts no longer wedge a node** - A disk mount left pointing at a deleted volume could spin in a tight loop that filled etcd and jammed the runtime on its next boot. The loop is fixed, and orphaned mounts like it are cleaned up automatically. ([#914](https://github.com/mirendev/runtime/pull/914))
- **Explicit local disks are recognized wherever they're mounted** - Declaring a local disk the normal way still warned that your `app.toml` had no disk configured and quietly added a second, duplicate mount. Miren now recognizes an explicitly declared local disk at any mount path. ([#930](https://github.com/mirendev/runtime/pull/930))
- **`route protect --help` shows the password-provider example again** - A duplicate registration had shadowed the fuller help text, dropping the example for protecting a route with a password provider. It's back in both `--help` and the docs. ([#931](https://github.com/mirendev/runtime/pull/931))
- **Section help works behind a global flag** - `miren -C prod auth help` showed top-level help instead of `auth`'s subcommands. Section help now works regardless of a preceding global flag. ([#908](https://github.com/mirendev/runtime/pull/908))
