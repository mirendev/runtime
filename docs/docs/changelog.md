...
- **Per-route request timeouts** - A slow endpoint — long-poll, a streaming response, a slow first byte — used to force you to raise the cluster-wide request timeout for every app just to keep that one route alive. Now a route carries its own: `miren route timeout longpoll.example.com 10m`, or `--clear` to fall back to the cluster default. ([#1057](https://github.com/mirendev/runtime/pull/1057))
...
- **Reach a cluster behind NAT through Miren Cloud** - A cluster that isn't directly reachable — behind NAT, on a home network, no public route — used to be hard to run `miren` against. Now it can be reached over the authenticated connection it already holds up to Miren Cloud: `miren cluster add --via-cloud`. Your credentials ride end-to-end inside the request; cloud only relays the bytes, it can't read them. ([#979](https://github.com/mirendev/runtime/pull/979), [#1056](https://github.com/mirendev/runtime/pull/1056))
...
- **Disconnect a cluster from Miren Cloud** - `miren server unregister` is the clean inverse of `register`: it revokes the cluster's cloud credentials, clears the local registration, and restarts. Pass `--local-only` to just forget locally. ([#1021](https://github.com/mirendev/runtime/pull/1021))
...
- **Clearer debug output** - `miren debug entity` was rebuilt around readable cards and tables and no longer chokes on large kinds. `miren debug saga` lists and inspects saga executions — timing, undo state, errors — without decoding base64 by hand. `miren debug etcdctl` talks to the embedded etcd with the endpoint and credentials filled in for you. ([#1030](https://github.com/mirendev/runtime/pull/1030), [#1026](https://github.com/mirendev/runtime/pull/1026), [#1023](https://github.com/mirendev/runtime/pull/1023))
...
**Documentation**
- **Minimum working examples on every feature page** - Every feature page now opens with a copy-pasteable minimal example, so you can see the shape of a thing before reading the details. ([#1046](https://github.com/mirendev/runtime/pull/1046))
