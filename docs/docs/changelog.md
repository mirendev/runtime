...
**Improvements**
- **The server can no longer take the machine down with it** - A goroutine leak grew the control process until the host ran out of memory and stopped responding to anything, including SSH. The systemd unit now carries a memory limit, so the kernel stops and restarts the server instead of letting it exhaust the machine. Apps and addon databases run in their own cgroups outside the server's, so the limit doesn't apply to them and they keep running through a restart. The limit is a quarter of the machine's memory with a floor of 2 GB, and lands in a drop-in that is refreshed on every upgrade — so existing installs pick it up on their next `miren server upgrade` with nothing to do by hand. Raise it with `sudo systemctl edit miren` if your cluster genuinely needs more; an override there wins and survives upgrades. ([#1210](https://github.com/mirendev/runtime/pull/1210))

  If you already set a memory limit by hand, move it to `sudo systemctl edit miren`: a limit written directly into `/etc/systemd/system/miren.service` is overridden by the managed drop-in, silently and possibly downward. See [Troubleshooting](./troubleshooting.md#the-server-restarted-on-its-own).

  The limit covers cached file data as well as the server itself, and on a busy machine most of it is cache: a production coordinator we measured held 39 GB of container image cache against 489 MB of its own memory. Cache is handed back automatically as the limit is approached, so this costs nothing on a machine with memory to spare — which is why the limit scales with the machine instead of stopping at a fixed ceiling.

  When a restart does happen, the server says so on its next start. `miren logs system` carries `previous miren server run was killed for exceeding its memory limit`, along with the memory it reached, the limit it was held to, and how many times systemd has restarted it. Previously the server came back up with no idea it had ever gone down.

**Breaking Changes**
- **Wildcard subdomains need a TLS check to get certificates** - A wildcard route used to get a Let's Encrypt certificate for any subdomain a client named, and so did one extra label in front of any route. Scanners took advantage by guessing names: one cluster had issued more than 500 certificates for hostnames nothing served. A name that isn't itself a route now gets a real certificate only when a live ephemeral deploy has that label, or when the route's app vouches for it through a TLS check, set with `miren route tls-check '*.example.com' /tls-check`. Any other name gets the fallback certificate. If you serve real tenants off a wildcard route, add a TLS check endpoint that answers `200` for the names you serve, or add an exact route for each one. Preview deploys need nothing new. ([MIR-1919](https://linear.app/miren/issue/MIR-1919))
...
**Improvements**
- **`miren deploy` output you can script against** - When stdout is a pipe, deploy output carried clear-line escape codes from the upload meter, replayed every build step, and buried the version in a sentence, so CI logs were noisy and scripts had to scrape prose. The non-terminal path now prints plain lines, condenses the build to one `Build & push image ... 5 steps, 3 cached` summary, and ends with `✓ Deploy successful` and a `Version: <id>` line. `--format json` puts one document with `status`, `app_version`, `deploy_id`, and `urls` on stdout (progress goes to stderr, and a failed deploy still reports with `status: failed`), `--format jsonl` streams the whole deploy as one JSON object per line (upload, build steps and logs, deployment phases, health, and a final `result`) with nothing else on either stream, and `--quiet` drops upload and build progress entirely. `miren debug deploy-events` turns a saved JSONL stream back into readable output, with `--timestamps` to see where the time went. ([MIR-660](https://linear.app/miren/issue/MIR-660))
- **Uploads no longer fail when a file changes size mid-deploy** - A file being appended to during the upload (a log, or the deploy's own output redirected into the project) grew past the size its archive header had promised, and the upload failed with `archive/tar: write too long`. The archive now carries such a file at the size it had when the deploy started and continues. ([MIR-660](https://linear.app/miren/issue/MIR-660))
