...
func TeaModel() tea.Model {
	cd := &consoleDisplay{
		t: newTrace(io.Discard, true),
		disp: &ttyDisplay{
			phase:  "Building",
			width:  80,
			height: 24,
...
func (m model) Init() tea.Cmd {
	return tea.Tick(time.Second, func(ts time.Time) tea.Msg {
		return ts
	})
}
...
func (m model) View() string {
	return m.cd.view(m.width, m.height)
}
...
func (d *consoleDisplay) update(ss *client.SolveStatus) {
	...
	if d.displayLimiter != nil && !d.displayLimiter.Allow() {
...
func (d *consoleDisplay) view(width, height int) string {
	return d.disp.view(d.t.displayInfo(), width, height, false)
}
...
func (disp *ttyDisplay) getSize() (int, int) {
	if disp.c != nil {
		...
		if err == nil && size.Width > 0 && size.Height > 0 {
			disp.width = int(size.Width)
			disp.height = int(size.Height)
		}
	}
...
func (disp *ttyDisplay) view(d displayInfo, width, height int, all bool) string {
	// this output is inspired by Buck
	d.jobs = setupTerminals(d.jobs, height-8, all)
	disp.repeated = true
	...
	*/
	...
	/*
		...
		if diff := disp.lineCount - lineCount; diff > 0 {
			for i := 0; i < diff; i++ {
				fmt.Fprintln(&ss, strings.Repeat(" ", width))
			}
			fmt.Fprint(&ss, aec.EmptyBuilder.Up(uint(diff)).Column(0).ANSI)
		}
		disp.lineCount = lineCount
	*/
