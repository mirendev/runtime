...
func TestEnumEmitsRefValues(t *testing.T) {
	sb := &SchemaBuilder{
		domain: "test-enum",
		attrs:  make(map[entity.Id]*entity.Entity),
	}

	id := sb.Enum("state", "test-enum/state", []entity.Id{
		"test-enum/state.ready",
		"test-enum/state.disabled",
	})
	...
	assert.Equal(t, entity.KindId, values[1].Kind())
}
