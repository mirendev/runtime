...
type schemaAttr struct {
	...
	Choices       []string `yaml:"choices,omitempty"`        // for inline enum attributes
	Enum          string   `yaml:"enum,omitempty"`           // named enum type
	LegacyChoices bool     `yaml:"legacy-choices,omitempty"` // preserve a former inline enum's names
...
type gen struct {
	...
	legacyConstants  map[string]int
...
func GenerateSchema(sf *schemaFile, pkg string) (string, error) {
	...
	usedAttrs := map[string]struct{}{}
	legacyConstants := legacyEnumConstantCounts(sf)
	for name, enum := range sf.Enums {
		for _, value := range enum.Values {
			usedAttrs[enumMemberID(sf, name, value)] = struct{}{}
		}
	}
	...
	for compName, attrs := range mapx.StableOrder(sf.Components) {
		...
		g.legacyConstants = legacyConstants
		g.isComponent = true
		g.name = compName
	...
	validateAttrs = func(path string, attrs schemaAttrs) error {
		for name, attr := range attrs {
			...
			if attr.Type == "enum" {
				...
				if attr.Enum == "" {
					if attr.LegacyChoices {
						return fmt.Errorf("inline enum field %s cannot declare legacy choices", fieldPath)
					}
					continue
				}
...
func legacyEnumConstantCounts(sf *schemaFile) map[string]int {
	counts := make(map[string]int)
	var countAttrs func(schemaAttrs)
	countAttrs = func(attrs schemaAttrs) {
		for _, attr := range attrs {
			if attr.Type == "enum" && attr.Enum != "" && attr.LegacyChoices {
				for _, value := range sf.Enums[attr.Enum].Values {
					counts[strings.ToUpper(value)]++
				}
			}
			countAttrs(attr.Attrs)
		}
	}

	// Standalone components already namespace their legacy constants. Kind
	// fields historically did not, so only they can collide at package scope.
	for _, attrs := range sf.Kinds {
		countAttrs(attrs)
	}
	return counts
...
func (g *gen) attr(name string, attr *schemaAttr) {
	...
	case "enum":
		...
		if attr.LegacyChoices {
			...
			g.decodeouter = append(g.decodeouter, j.Const().DefsFunc(func(group *j.Group) {
				for _, value := range enum.Values {
					...
					} else if g.legacyConstants[legacyConstant] > 1 {
						legacyConstant = g.local + fname + legacyConstant
					}
		...
		legacyValues := make(map[string][]entity.Id, len(enum.Values))
		legacyRefIDs := make(map[string]string, len(enum.Values))
		legacyGoValues := make(map[string]string, len(enum.Values))
		...
		for _, value := range enum.Values {
			...
			if attr.LegacyChoices {
				legacyID := name + "." + value
				legacyGoValue := name + "." + value
				if g.isComponent {
					legacyID = attr.Attr + "." + value
					legacyGoValue = attr.Attr + "." + value
				}
				legacyID = g.sf.Domain + "/" + legacyID
				legacyRefIDs[value] = legacyID
				legacyGoValues[value] = legacyGoValue
				...
				if legacyID != canonicalID {
					legacyValues[value] = append(legacyValues[value], entity.Id(legacyID))
				}
			}
		}
		...
		g.decodeouter = append(g.decodeouter, j.Var().Id(mapName+"ToId").Op("=").Map(j.Id(enumType)).Qual(top, "Id").ValuesFunc(func(b *j.Group) {
			for _, value := range enum.Values {
				...
				if legacyGoValue := legacyGoValues[value]; legacyGoValue != "" && legacyGoValue != value {
					b.Id(enumType).Call(j.Lit(legacyGoValue)).Op(":").Id(enumMemberConstant(attr.Enum, value))
				}
			}
		}))
		g.decoders = append(g.decoders, j.If(
		...
		enumCall := []j.Code{call[0], call[1], j.Index().Qual(top, "Id").Values(canonicalSchemaValues...)}
	...
	case "component":
		...
		sg.legacyConstants = g.legacyConstants
