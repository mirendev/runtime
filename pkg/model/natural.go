...
func decodeNaturalValue(f *entity.SchemaField, v any) ([]entity.Attr, error) {
	...
	// Named enums choose their canonical physical value through the encoded
	// schema. Older schemas represented ref-backed enums solely as Type "enum"
	// plus EnumValues; SchemaField.EnumValue continues to handle those as well.
	if f.Enum != "" || f.Type == "enum" {
		...
		if !ok {
			members := make([]string, 0, len(f.EnumValues))
			for member := range f.EnumValues {
				members = append(members, member)
			}
			slices.Sort(members)
			return nil, fmt.Errorf("failed to decode field %s: enum value %q is not declared, expected one of %v", f.Name, enum, members)
		}
