...
// ProxyStats contains tracking data for a proxied request
...
// ProxyCallback is called after the response is complete with tracking data
type ProxyCallback func(stats ProxyStats)
...
type ReverseProxy struct {
	...
	// Callback is an optional function that is called after the response
	// is complete with tracking data including status code, bytes written,
	// and duration.
...
func (p *ReverseProxy) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	// Set up tracking if callback is configured
	if p.Callback != nil {
		startTime = time.Now()
		enableTracking = true
	}
	...
	responseBytes, err := p.copyResponse(rw, res.Body, p.flushInterval(res))
	if err != nil {
...
func (p *ReverseProxy) copyResponse(dst http.ResponseWriter, src io.Reader, flushInterval time.Duration) (int64, error) {
	var w io.Writer = dst
	...
	written, err := p.copyBuffer(w, src, buf)
	return written, err
}
