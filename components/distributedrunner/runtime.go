...
// Runtime owns the started distributed-runner graph and its role-level handle.
type Runtime struct {
	graph *boot.Graph
	once  sync.Once
...
func Start(options StartOptions) (*Runtime, error) {
	...
	options.Log.Info("runner started successfully",
		"runner_id", options.Config.RunnerID,
		"listen_address", options.ListenAddr)
	return runtime, nil
}
