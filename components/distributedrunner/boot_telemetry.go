...
func newTelemetryBoot(inputs telemetryBootInputs) *telemetryBoot {
	b := &telemetryBoot{inputs: inputs}
	b.component, b.output = boot.Provide0("telemetry", b.start,
		boot.WithStop(b.stop, 0))
	return b
}

func (b *telemetryBoot) start(context.Context) (telemetryBootOutput, error) {
	...
	if b.inputs.victoriaMetricsAddress != "" || b.inputs.victoriaLogsAddress != "" {
		result.tokenSource = runnertelemetry.NewIssuerTokenSource()
