...
import (
	...
	"miren.dev/runtime/api/schedule/v1alpha"
...
func (s *Scheduler) gatherNodes(ctx context.Context, eac *eas.EntityAccessClient) ([]*ns.Node, error) {
	results, err := eac.List(ctx, entity.Keyword(entity.EntityKind, ns.KindNode))
	if err != nil {
		return nil, err
	}
...
func NewScheduler(ctx context.Context, log *slog.Logger, eac *eas.EntityAccessClient) (*Scheduler, error) {
	s := &Scheduler{
		log:   log,
		nodes: make(map[entity.Id]*ns.Node),
	}
...
func (s *Scheduler) FindNodeById(id entity.Id) (*ns.Node, error) {
	node, ok := s.nodes[id]
	if !ok {
...
func (s *Scheduler) assignSandbox(ctx context.Context, ent *entity.Entity, eac *eas.EntityAccessClient) error {
	...
	if _, ok := sandbox.Get(v1alpha.KeyId); ok {
		return nil
	}
