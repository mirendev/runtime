...
func EntityPatch(ctx *Context, opts struct {
	...
	for _, path := range opts.Path {
		...
		for _, rpcEnt := range pf.Entities() {
			...
			if opts.DryRun {
				ctx.Log.Info("Dry run, not patching entity", "id", opts.Id, "attrs", attrs)
				continue
			}

			res, err := eac.Patch(ctx, attrs, opts.Revision)
			if err != nil {
				return fmt.Errorf("failed to patch entity: %v", err)
			}

			ctx.Log.Info("Entity patched successfully", "id", res.Id, "revision", res.Revision)
		}
	}
