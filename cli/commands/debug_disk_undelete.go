...
// DebugDiskUndelete recovers a deleted disk by moving its data back and
// recreating its entities directly, without going through the server.
//
// Break-glass, for when the server's RPC listener is down; `miren disk
// undelete` is the supported command. It therefore has to run on the server and
// keeps --data-path.
func DebugDiskUndelete(ctx *Context, opts struct {
	ConfigCentric
