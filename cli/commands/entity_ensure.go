...
func EntityEnsure(ctx *Context, opts struct {
	...
	for _, path := range opts.Path {
		...
		for _, rpcEnt := range pf.Entities() {
			...
			if opts.DryRun {
				ctx.Log.Info("Dry run, not ensuring entity", "id", opts.Id, "attrs", attrs)
				continue
			}

			res, err := eac.Ensure(ctx, attrs)
			if err != nil {
				return fmt.Errorf("failed to ensure entity: %v", err)
			}

			if res.Created() {
				ctx.Log.Info("Entity created successfully", "id", res.Id(), "revision", res.Revision())
			} else {
				ctx.Log.Info("Entity already exists", "id", res.Id(), "revision", res.Revision())
			}
		}
	}
