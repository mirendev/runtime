...
// DebugDiskBackup backs up a disk by reading its image directly, without going
// through the server.
//
// This is break-glass, not a second supported way to back up. `miren disk
// backup` drives the server over RPC and is the command to use. This one exists
// for the case that one cannot cover: the server's RPC listener being down on a
// host you can still get a shell on. It therefore has to run on the server and
// keeps --data-path.
func DebugDiskBackup(ctx *Context, opts struct {
	ConfigCentric
