...
func Dev(ctx *Context, opts struct {
	...
	if opts.Mode == "standalone" && !opts.SkipClientConfig {
		if err := writeDevClusterConfig(ctx, opts.Address); err != nil {
...
func writeDevClusterConfig(ctx *Context, address string) error {
	...
	// Create config content with insecure set to true for dev
	configContent := fmt.Sprintf(`# Auto-generated by 'runtime dev' - DO NOT EDIT
clusters:
  dev:
    hostname: %s
    insecure: true
`, address)
	...
	if err := os.WriteFile(devConfigPath, []byte(configContent), 0644); err != nil {
		return fmt.Errorf("failed to write dev config file: %w", err)
	}

	ctx.Log.Info("wrote dev cluster config", "path", devConfigPath, "address", address)
	return nil
}
