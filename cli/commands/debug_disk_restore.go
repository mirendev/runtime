...
// DebugDiskRestore restores a disk by writing its image directly, without going
// through the server.
//
// Break-glass, for when the server's RPC listener is down; `miren disk restore`
// is the supported command. Note that this path cannot see whether the image is
// currently loop-attached, so it can rename a restored image over a live one and
// report success having changed nothing. `miren disk restore` refuses that case.
func DebugDiskRestore(ctx *Context, opts struct {
	ConfigCentric
