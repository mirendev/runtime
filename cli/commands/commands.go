...
`,
	...
	d.Dispatch("server operations", Section("server operations", "Durable server restart and upgrade operations", ""))
	d.Dispatch("server operations list", Infer("server operations list", "List recorded restart and upgrade operations", ServerOperationsList,
		WithExample(mflags.Example{
			Name: "List operations",
			Body: "miren server operations list",
		}),
	))
	d.Dispatch("server operations show", Infer("server operations show", "Show one restart or upgrade operation", ServerOperationsShow,
		WithExample(mflags.Example{
			Name: "Show an operation",
			Body: "miren server operations show 01J8X2M0QK4V6Z9W1N3RB5T7YC",
		}),
	))
